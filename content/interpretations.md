...
It seems that there is not a strong concensus about the distinctions between calculational [[tools vs models]], and which interpretations belong in which category. This results in interpretations that try to make physical sense of something that might otherwise be recognized as a convenient mathematical tool for computing answers to specific kinds of physics problems.
...
Two of the most important issues that different interpretations wrestle with are the apparent [[non-locality]] of quantum physics, and the status of the exponentially-large [[configuration space]] (which is also manifestly non-local) that is used in most mathematical frameworks, including the [[pilot-wave]] framework. Each of these effectively invalidates any attempt to develop an intuitively appealing physical model along the lines of the Lorenz gauge version of EM and general relativity. This can be concretely understood in the context of the [[cellular automaton]] (CA) framework, which provides a specific set of properties that are consistent with what people generally agree as being physically plausible.

Indeed, the conflict between quantum physics and [[gravity]] is widely recognized as the single greatest unsolved problem in physics today, and despite many years of trying, it is not clear that significant progress is being made. Given how strongly validated general relativity is empirically, this may indicate that the fault lies with our current understanding of quantum physics (which is also obviously strongly validated empirically), particularly with respect to these issues of non-locality and the exponentially-large size of configuration space.
