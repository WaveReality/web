...
As discussed in [[field theory]], this framework is based on the convenient [[tools vs models|calculational tool]] of Fourier space, which represents the momentum, but not position, of the relevant particles. Thus, it only provides a partial window into the actual underlying physical phenomena, and is overall quite misleading if adopted as a physical model.
